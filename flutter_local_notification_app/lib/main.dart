...
Future<void> main() async {
  ...
  final IOSInitializationSettings initializationSettingsIOS = IOSInitializationSettings(
      ...
      onDidReceiveLocalNotification: (      // display a dialog with the notification details, tap OK to go to another page
          int id,
          String? title,
          String? body,
          String? payload,
...
class PaddedElevatedButton extends StatelessWidget {
  ...
  const PaddedElevatedButton({Key? key, required this.buttonText, required this.onPressed,}) : super(key: key);
...
class HomePage extends StatefulWidget {
  ...
  const HomePage(this.notificationAppLaunchDetails, {Key? key,}) : super(key: key);
...
class _HomePageState extends State<HomePage> {
          ...
          actions: <Widget>[
            CupertinoDialogAction(
              ...
              onPressed: () async {
                ...
                await Navigator.push(context, MaterialPageRoute<void>(builder: (BuildContext context) => SecondPage(receivedNotification.payload),),);
              },
              child: const Text('Ok'),
            )
          ],
          ...
          child: Center(
            child: Column(
              children: <Widget>[
                ...
                PaddedElevatedButton(
                  buttonText:
                  'Show plain notification that has no title with payload',
                  onPressed: () async {
                ...
                if (kIsWeb || !Platform.isLinux) ...<Widget>[
                  ...
                  PaddedElevatedButton(
                    buttonText:
                    'Schedule weekly Monday 10:00:00 am notification in your local time zone',
                    onPressed: () async {
                ...
                PaddedElevatedButton(
                  buttonText:
                  'Schedule monthly Monday 10:00:00 am notification in your local time zone',
                  onPressed: () async {
                ...
                PaddedElevatedButton(
                  buttonText:
                  'Schedule yearly Monday 10:00:00 am notification in your local time zone',
                  onPressed: () async {
                ...
                if (!kIsWeb && Platform.isAndroid) ...<Widget>[
                  ...
                  PaddedElevatedButton(
                    buttonText:
                    'Show notification with custom vibration pattern, red LED and red icon',
                    onPressed: () async {
                ...
                if (!kIsWeb && Platform.isLinux) ...<Widget>[
                  ...
                  FutureBuilder<LinuxServerCapabilities>(
                    ...
                    builder: (BuildContext context, AsyncSnapshot<LinuxServerCapabilities> snapshot,) {
                      if (snapshot.hasData) {
                  ...
                  PaddedElevatedButton(
                    buttonText: 'Show notification on different screen location',
                    onPressed: () async {
  ...
  Future<void> _showFullScreenNotification() async {
    await showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text('to see the full-screen intent in 5 seconds, press OK and TURN OFF your screen'),
        actions: <Widget>[
          TextButton(
            onPressed: () {
  ...
  Future<void> _showMessagingNotification() async {
    ...
    await Future<void>.delayed(const Duration(seconds: 10), () async {
      messages.add(
        Message("I'm so sorry!!! But I really like thai food ...", DateTime.now().add(const Duration(minutes: 11)), null,),
      );
      await flutterLocalNotificationsPlugin.show(
        0,
        'message title',
  ...
  Future<void> _showNotificationsWithThreadIdentifier() async {
    NotificationDetails buildNotificationDetailsForThread(String threadIdentifier,) {
  ...
  Future<void> _createNotificationChannelGroup() async {
    ...
    await showDialog<void>(
      context: context,
      builder: (BuildContext context) => AlertDialog(
        content: Text('Channel group with name ${androidNotificationChannelGroup.name} created'),
        actions: <Widget>[
          TextButton(
            onPressed: () {
              Navigator.of(context).pop();
  ...
  Future<void> _createNotificationChannel() async {
    ...
    await showDialog<void>(
        ...
        builder: (BuildContext context) => AlertDialog(
          content:
          Text('Channel with name ${androidNotificationChannel.name} created'),
          actions: <Widget>[
            TextButton(
              onPressed: () {
                Navigator.of(context).pop();
  ...
  Future<Widget> _getActiveNotificationsDialogContent() async {
    ...
    if (!(androidInfo.version.sdkInt >= 23)) {
      return const Text('"getActiveNotifications" is available only for Android 6.0 or newer',);
    }
...
Future<void> _showLinuxNotificationWithByteDataIcon() async {
  final ByteData assetIcon = await rootBundle.load('icons/app_icon_density.png',);
...
Future<void> _showLinuxNotificationWithThemeIcon() async {
  final LinuxNotificationDetails linuxPlatformChannelSpecifics = LinuxNotificationDetails(icon: ThemeLinuxIcon('media-eject'),);
  final NotificationDetails platformChannelSpecifics = NotificationDetails(linux: linuxPlatformChannelSpecifics,);
  await flutterLocalNotificationsPlugin.show(
    0,
...
Future<void> _showLinuxNotificationWithThemeSound() async {
  final LinuxNotificationDetails linuxPlatformChannelSpecifics = LinuxNotificationDetails(sound: ThemeLinuxSound('message-new-email'),);
  final NotificationDetails platformChannelSpecifics = NotificationDetails(linux: linuxPlatformChannelSpecifics,);
  await flutterLocalNotificationsPlugin.show(
    0,
...
Future<void> _showLinuxNotificationWithCriticalUrgency() async {
  const LinuxNotificationDetails linuxPlatformChannelSpecifics = LinuxNotificationDetails(urgency: LinuxNotificationUrgency.critical,);
...
Future<void> _showLinuxNotificationResident() async {
  const LinuxNotificationDetails linuxPlatformChannelSpecifics = LinuxNotificationDetails(resident: true,);
  const NotificationDetails platformChannelSpecifics = NotificationDetails(linux: linuxPlatformChannelSpecifics,);
  await flutterLocalNotificationsPlugin.show(
    0,
...
Future<void> _showLinuxNotificationDifferentLocation() async {
  const LinuxNotificationDetails linuxPlatformChannelSpecifics = LinuxNotificationDetails(
    location: LinuxNotificationLocation(10, 10),
  );
  const NotificationDetails platformChannelSpecifics = NotificationDetails(linux: linuxPlatformChannelSpecifics,);
  await flutterLocalNotificationsPlugin.show(
    0,
...
class _InfoValueString extends StatelessWidget {
  const _InfoValueString({Key? key, required this.title, required this.value,}) : super(key: key);
